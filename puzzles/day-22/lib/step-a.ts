...
function countUnnecessaryBricks(): number {
	let n = 0;

	for (const brick of Object.values(bricks)) {
		if (!isBrickNecessary(brick)) n++;
	}

	return n;
}

function isBrickNecessary(brick: Brick): boolean {
	for (const id of brick.mounteds) {
		const mounted = bricks[id];
		if (!mounted) throw new Error("No mounted, something is wrong.");
