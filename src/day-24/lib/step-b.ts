/**
 * This solution is from shahata5: https://www.reddit.com/r/adventofcode/comments/18pnycy/comment/khlrstp/?utm_source=share&utm_medium=web3x&utm_name=web3xcss&utm_term=1&utm_content=share_button
 *
...
let hails: Array<Array<BigInt>> = [];

export function run(input: Array<string>): bigint {
	init(input);

	const A: Array<[bigint, bigint, bigint, bigint, bigint, bigint]> = [];
	const B: Array<bigint> = [];
...
function init(input: Array<string>): void {
	hails = input.map(
		(line) => line.match(/-?\d+/g)?.map(BigInt) as Array<BigInt>,
	);
}

function add(
	A: Array<[bigint, bigint, bigint, bigint, bigint, bigint]>,
	B: Array<bigint>,
	n: number,
): void {
	const [px0, py0, pz0, vx0, vy0, vz0] = hails[0] as [
		bigint,
		bigint,
		bigint,
		bigint,
	...
	const [pxN, pyN, pzN, vxN, vyN, vzN] = hails[n] as [
		bigint,
		bigint,
		bigint,
		bigint,
...
function cramer(
	...
	return A.map(
		(_, i) =>
			det(
				A.map((r, j) => r.toSpliced(i, 1, B[j] as bigint)) as [
					bigint,
					bigint,
					bigint,
					bigint,
...
function det(
	m: Array<[bigint, bigint, bigint, bigint, bigint, bigint]>,
): bigint {
	...
	const n = l.map(
		(n, i) =>
			n *
			det(
				r.map((row) => row.toSpliced(i, 1)) as Array<
					[bigint, bigint, bigint, bigint, bigint, bigint]
				>,
			),
	);

	return n.reduce((a, b, i) => (i % 2 ? a - b : a + b), 0n);
}
